...
class Db
{
	...
	public function __construct($connection_name = 'default')
	{
		if($connection_name == 'default')
			$connection_name = Config::find('connection', 'default');
		...
		if(!in_array($connection_name, Config::find('connection', 'enabled')))
			throw new \Exception("Connection for $connection_name is not enabled");

		$this->config = Config::get('connection', 'available', $connection_name);
	...
	{
		return $this->service_class->commit($this->get_connection());
		$this->stop_private();
	}

	public function rollback()
	{
		return $this->service_class->rollback($this->get_connection());
		$this->stop_private();
	}
